...
func (gores *Gores) BlockPop(queues mapset.Set) (string, map[string]string, error) {
//func (gores *Gores) BlockPop(queues mapset.Set) (string, map[string]interface{}, error) {
    ...
    `
	for elem := range it.C {
        hashtag_elem := fmt.Sprintf("{%s}", elem)
        curQueueKey := fmt.Sprintf(queuePrefix,hashtag_elem)
        data, err := conn.Do("EVAL", lua, 2, curQueueKey, "mq:wip:xx")
        if err != nil {
        ...
        msgKey := fmt.Sprintf("mq:msg:%s", data)
        // https://stackoverflow.com/questions/29517213/redigo-read-redis-hash-that-has-variable-keys
        msgData, err := redis.StringMap(conn.Do("HGETALL", msgKey))
        if err != nil {
            return "", nil, fmt.Errorf("get msg failed: %s", err)
        }
        fmt.Println(msgData)

        // returned data contains [key, value], extract key at index 0, value at index 1
        queueKey := msgData["origin"]
        fmt.Println(queueKey)

        //decoded, _ := gores.Decode(data.([]interface{})[1].([]byte))
        return queueKey, msgData, nil
	}
